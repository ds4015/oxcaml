...
let paint_quilt_board (qb : Game_board.quilt_board) qb_repl ext_qb =
  ...
  let rec iterate_pip patches =
    ...
    | (row, col, (patch : Patch.patch_shape)) :: tl ->
        let x = x_base +. (x_step *. float_of_int (col - 1)) in
        ...
        let h = float_of_int num_rows *. x_step in
        ...
        image##setAttribute (Js.string "preserveAspectRatio") (Js.string "xMidYMid meet");
...
let show_cursor_img ~src ~(cols : int) =
  let cursor_img = Dom_html.getElementById_exn "cursor-patch-img" in
        ...
        | Some btn ->
            btn##.onclick :=
              Dom_html.handler (fun _ ->
                  ...
                  let patch_name =
                    find_patch_name (unwrap_exn !current_state).patches patch_index
                  in
                  let patch_shape =
                  ...
                  let img_src = "svgs/" ^ patch_name ^ ".svg" in
                  show_cursor_img ~src:img_src ~cols:patch_cols;
                          ...
                          | Some c, Some r ->
                              ...
                              let updated_state =
                                Move.choose_move (unwrap_exn !current_state) PlacePatch
                                  patch_index (int_of_string row) (int_of_string column)
                              in
                              current_state := Some updated_state;
                              disable_taken_patch_img patch_index;
