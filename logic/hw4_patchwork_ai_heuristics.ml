...
let test_board_cells patch (board : Game_board.quilt_board) =
  ...
  for x = 1 to board.squares do
    for y = 1 to board.squares do
      let valid_cell =
        Game_board.check_if_patch_fits (Patch.get_patch_dim patch) board x y
      in
...
let check_for_holes (board : Game_board.quilt_board) patch cell_row cell_col =
  ...
  let rec walk_adj_empties adj_e acc =
    ...
    | (re, ce) :: tl ->
        ...
        in
        if up_occ_or_oob && down_occ_or_oob && left_occ_or_oob && right_occ_or_oob then
          walk_adj_empties tl (acc + 1)
...
let best_score_for_patch (board : Game_board.quilt_board)
    (patch_shape : Patch.patch_shape) (patch_cost : int) (patch_time : int)
    (patch_income : int) (extra_turn : float) (valid_cells : (int * int) list) :
    float * (int * int) =
  let rec loop (cells : (int * int) list) (best_score : float) (best_cell : int * int) :
    ...
    | (r, c) :: tl ->
        ...
        let score : float =
          ...
          -. float_of_int holes
        in
        if Float.(score > best_score) then loop tl score (r, c)
...
let determine_move (current_state : Game_state.t) =
  ...
  let move_penalty =
    if buttons_gained > 3 then -2
    else if buttons_gained > 6 then -4
    else if buttons_gained > 9 then -5
    else 0
  in
  ...
  let (p1_best_score : float), p1_best_cell =
    if afford_patch_1 < 0 then (Float.neg_infinity, (1, 1))
    else
      best_score_for_patch current_state.p2qb patch_1.shape patch_1_cost patch_1_time
        patch_1_income patch_1_extra_turn_bonus valid_cells_to_place_patch_1
  in
  let (p2_best_score : float), p2_best_cell =
    if afford_patch_2 < 0 then (Float.neg_infinity, (1, 1))
    else
      best_score_for_patch current_state.p2qb patch_2.shape patch_2_cost patch_2_time
        patch_2_income patch_2_extra_turn_bonus valid_cells_to_place_patch_2
  in
  let (p3_best_score : float), p3_best_cell =
    if afford_patch_3 < 0 then (Float.neg_infinity, (1, 1))
    else
      best_score_for_patch current_state.p2qb patch_3.shape patch_3_cost patch_3_time
        patch_3_income patch_3_extra_turn_bonus valid_cells_to_place_patch_3
  in
  ...
  if Float.(best_score >= advance_heuristic) then
    (Move.PlacePatch, best_patch.pos_around_board, fst best_cell, snd best_cell)
  else (Advance, 0, 0, 0)
